...
// ExampleStreamReader_ReadSeriesWithIndexedInfo_Writer shows us to use an
// `Iterator` to read a stream footer and then using the index-info structs to
// retrieve specific series rather than iterating through them one at a time.
func ExampleStreamReader_ReadSeriesWithIndexedInfo_Writer() {
...
// ExampleStreamReader_ReadSeriesWithIndexedInfo_Datasource shows us to use an
// `Iterator` to read a stream footer and then using the index-info structs to
// retrieve specific series rather than iterating through them one at a time.
func ExampleStreamReader_ReadSeriesWithIndexedInfo_Datasource() {
	b := rifs.NewSeekableBuffer()
	...
	sddrw := NewSeriesDataDatasourceReaderWrapperFromWriter(seriesData)

	seriesFooter, _, checksumOk, err = sr.ReadSeriesWithIndexedInfo(sisi, sddrw)
	log.PanicIf(err)
